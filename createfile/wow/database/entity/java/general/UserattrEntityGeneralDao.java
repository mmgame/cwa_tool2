...
public class UserattrEntityGeneralDao implements IUserattrEntityDao {
	protected static String namespace = UserattrEntity.class.getCanonicalName();

	public UserattrEntityGeneralDao() {
	}
	...
	public UserattrEntity selectEntityByUserIdAndAttrType(long userId,int attrType,Object attach) {
		...
		paras.put("attrType", attrType);
		SqlSession sqlSessionTemplate = (SqlSession) paras.get("sql_session");
		List<UserattrEntity> list = sqlSessionTemplate.selectList(namespace + ".selectByUserIdAndAttrType", paras);
